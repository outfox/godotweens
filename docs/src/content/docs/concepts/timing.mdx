...
| `Repeats` | `0` | Cycles after the first. `TweenOptions.Infinite` (-1) repeats until cancelled; a fully zero-time infinite cycle is rejected. |
...
`RepeatInterval = 0.3`, and `Repeats = 1` take 2.7 seconds with zero delay and
