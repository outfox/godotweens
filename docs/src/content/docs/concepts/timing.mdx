...
repeats. Most tweens only need `Duration`, `Delay`, and an ease. Everything else
here is for loops and edge cases.

## Loop forever

For idle motion such as bobbing or pulsing, use one definition that ping-pongs
forever. It stops by itself when `sprite` leaves the tree:
...
[shader uniforms](/csharp/shaders/)).
