...
description: Use the handle to pause, resume, or cancel a tween and find out why it ended. Also covers callbacks, errors, and threading.
---
...
latest value. To cancel every tween a node owns, call `node.CancelTweens()`.
...
Continue a sequence only on `Completed`, as [sequences](/csharp/sequences/)
shows. Godot emits tree exit before it invalidates a node freed with
...
of skipped cycles. Tweens created in `OnUpdate` begin on the next update, while
tweens created in `OnEnd` continue the finished tween's timeline exactly.
